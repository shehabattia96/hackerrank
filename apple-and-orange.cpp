// challenge at https://www.hackerrank.com/challenges/apple-and-orange/problem
#include <bits/stdc++.h>
...
void countApplesAndOranges(const int houseBegin, const int houseEnd, const int appleTree, const int orangeTree, const vector<int> apples, const vector<int> oranges) {
    int numberOfApplesOnHouse = 0, numberOfOrangesOnHouse = 0;
    int distance;
    for (int appleDistance : apples) {
        distance = appleTree + appleDistance;
        if (distance >= houseBegin && distance <= houseEnd)
            numberOfApplesOnHouse++;
    }
    for (int orangeDistance : oranges) {
        distance = orangeTree + orangeDistance;
        if (distance >= houseBegin && distance <= houseEnd)
            numberOfOrangesOnHouse++;
    }
    
    std::cout << numberOfApplesOnHouse << std::endl;
    std::cout << numberOfOrangesOnHouse << std::endl;
}
